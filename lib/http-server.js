...
class HttpServer {
    ...
    constructor(options) {
        ...
        //代理server
        let proxyServer = httpProxy.createServer({changeOrigin: true, secure: false});
            ...
            before.push((req, res) => {
                ...
                if (target) {
                    if (isUrl(target)) {
                        ...
                        target = hasExtension(target) ? target : path.join(target, HOME_PAGE);
                        let resolvePath = path.resolve(options.root, target);
                        try {
                            fs.lstatSync(resolvePath);
