...
const getLocalUrlOptions = (url) => {
    let result = {};
    const index = url.indexOf('#copy');
    if(index !== -1) {
        result.watch = url.indexOf('#watch') !== -1;
        result.url = url.replace('#copy', '').replace('#watch', '');
        result.copy = true;
    }else{
        result.watch = false;
        result.url = url.replace('#watch', '');
        result.copy = false;
    }
    return result;
}
    ...
    constructor(options) {
        ...
        if (options.local) {
            ...
            for (let key in local) {
                ...
                const opts = getLocalUrlOptions(local[key]);
                srcPath = path.isAbsolute(opts.url) ? path.normalize(opts.url) : path.normalize(path.join(base, options.root, opts.url));
                if(opts.copy) {
                    fileUtil.removeFile(key);
                    fileUtil.copyDir(srcPath, key);
                    if(opts.watch){
                        const globSrcFiles = glob([path.join(srcPath, '**')]);
                        const globTargetFiles = glob([path.join(key, '**')]);
                        gaze(globSrcFiles, function () {
                           this.on('changed', function (filePath) {
                               logger.debug('file changed: ', filePath);
                               const targetFilePath = path.join(key, path.relative(opts.url, filePath));
                               fileUtil.copyFile(filePath, targetFilePath);
                           });
                        });
                        gaze(globTargetFiles, function(){
                            this.on('changed', function (filePath) {
                                logger.debug('file changed: ', filePath);
                                const targetFilePath = path.join(opts.url, path.relative(key, filePath));
                                fileUtil.copyFile(filePath, targetFilePath);
                            });
                        });
                    }
